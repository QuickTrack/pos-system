...
export async function POST(
  ...
  try {
    ...
    if (!hasPermission(user.role as Role, 'finalize_payroll')) {
    ...
    if (run.status !== 'approved') {
      return NextResponse.json(
        { success: false, error: `Cannot finalize payroll run in status ${run.status}` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to finalize payroll run' },
      { status: 500 }
    );
  }
}
